(() => {
  ...
  function buildZXingReader(){
    ...
    try{const hints=new Map();hints.set(ZXing.DecodeHintType.POSSIBLE_FORMATS,[ZXing.BarcodeFormat.DATA_MATRIX,ZXing.BarcodeFormat.QR_CODE]);hints.set(ZXing.DecodeHintType.TRY_HARDER,true);zxingReader=new ZXing.MultiFormatReader();zxingReader.setHints(hints);return true;}catch(_){zxingReader=null;return false;}
  }

  async function buildNativeDetector(){
    detector=null;if(!('BarcodeDetector'in window))return false;
    try{const supported=await BarcodeDetector.getSupportedFormats();const formats=[];if(supported.includes('data_matrix'))formats.push('data_matrix');if(supported.includes('qr_code'))formats.push('qr_code');if(!formats.length)return false;detector=new BarcodeDetector({formats});return true;}catch(_){return false;}
  }

  function drawCrop(scale=.66){
    const vw=video.videoWidth||0,vh=video.videoHeight||0;if(!vw||!vh||!ctx)return false;
    const side=Math.max(180,Math.floor(Math.min(vw,vh)*scale));const sx=Math.max(0,Math.floor((vw-side)/2));const sy=Math.max(0,Math.floor((vh-side)/2));const target=scale<.58?720:640;
    canvas.width=target;canvas.height=target;ctx.imageSmoothingEnabled=true;ctx.drawImage(video,sx,sy,side,side,0,0,target,target);return true;
  }

  function imageDataToLuminance(imageData){const rgba=imageData.data;const out=new Uint8ClampedArray(imageData.width*imageData.height);for(let i=0,p=0;i<rgba.length;i+=4,p++)out[p]=((rgba[i]*77+rgba[i+1]*150+rgba[i+2]*29)>>8);return out;}

  function decodeZXingCanvas(){
    if(!zxingReader||!ctx)return'';const imageData=ctx.getImageData(0,0,canvas.width,canvas.height);const lum=imageDataToLuminance(imageData);const source=new ZXing.RGBLuminanceSource(lum,canvas.width,canvas.height);
    const tries=[()=>new ZXing.BinaryBitmap(new ZXing.HybridBinarizer(source)),()=>typeof ZXing.GlobalHistogramBinarizer==='function'?new ZXing.BinaryBitmap(new ZXing.GlobalHistogramBinarizer(source)):null,()=>new ZXing.BinaryBitmap(new ZXing.HybridBinarizer(source.invert()))];
  ...
  async function tryNative(){
    if(!detector||busyNative||video.readyState<2||paused)return false;busyNative=true;
    try{const codes=await detector.detect(video);for(const c of codes||[])if(handoffRaw(c.rawValue||'','camera-native'))return true;if(drawCrop(.62)){const codes2=await detector.detect(canvas);for(const c of codes2||[])if(handoffRaw(c.rawValue||'','camera-native-crop'))return true;}}catch(_){}finally{busyNative=false;}return false;
  }

  async function tryZXing(){
    if(busyZXing||video.readyState<2||paused)return false;if(!zxingReader&&!buildZXingReader())return false;busyZXing=true;
  ...
  async function setupTrack(){
    ...
    if(caps.torch)torchBtn?.classList.remove('hidden');if(caps.zoom&&zoom&&zoomWrap){zoom.min=caps.zoom.min;zoom.max=caps.zoom.max;zoom.step=caps.zoom.step||.1;const ideal=Math.min(Math.max(1.7,caps.zoom.min),caps.zoom.max);zoom.value=ideal;zoomWrap.classList.remove('hidden');try{await track.applyConstraints({advanced:[{zoom:ideal}]});}catch(_){}}
  }
  ...
  zoom?.addEventListener('input',event=>{if(!stream)return;event.stopImmediatePropagation();applyZoom();},true);
