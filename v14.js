(() => {
  ...
  function photoUrl(fig) {
    if (!fig || Number(fig.seriesId?.split('-')[1]) < 25) return '';
  ...
  let latestVersion=APP_VERSION, latestBuild=150;
  ...
  const infoCard=document.querySelector('#infoView .info-card');if(infoCard&&!infoCard.querySelector('.image-credit')){const credit=document.createElement('div');credit.className='image-credit';credit.innerHTML='Photos des séries récentes chargées depuis le projet open source <strong>L-Scan</strong>. Les marques et visuels LEGO restent la propriété de leurs ayants droit.';infoCard.appendChild(credit);}
})();
